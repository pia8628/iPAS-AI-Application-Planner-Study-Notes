...
6. **L2 正則化（Ridge）**
	1. 讓所有參數都「變小一點」。
	2. 縮小整體權重，避免模型過度依賴單一特徵。
	3. 邏輯是：「每個特徵都可能有點用，但不要有人太誇張。」
	4. 較保守、穩定、泛化能力強 
7. **Dropout** 隨機失活 ^1dec2a
	1. 在訓練過程中，**隨機關閉一部分神經元（neuron）**，讓模型不會過度依賴某些特定特徵。(讓神經元隨機「輪休」，強迫剩下成員協同合作，不能依賴幾個「明星神經元」，而讓模型更穩健)
	2. 某些神經元可能會形成 **強依賴關係**，例如：「n3 完全依賴 n2」但 Dropout 可能會把 n2 關掉（n 2=0），因此模型被迫學習更分散、更穩定的特徵表示，這會提升模型的 **泛化能力（generalization）**。
