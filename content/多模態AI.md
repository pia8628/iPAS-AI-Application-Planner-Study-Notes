...
訓練技巧：使用 **配對資料**（圖-文、音-文），並用 **對比損失** 讓正負樣本分開；大型模型可用 **指令微調(Instruction Tuning)** 讓模型遵從多模態指令。安全與合規需加 **內容審核、版權檢查、拒答策略**。效能優化可用 ** [LoRA](../00.%20Inbox/LoRA%20低秩適應.md) /Adapter** 微調、**量化蒸餾** 降低推論成本。資料標註成本高，常結合 **半監督、合成資料增強** 與 **偏差檢測**。
# 應用領域
